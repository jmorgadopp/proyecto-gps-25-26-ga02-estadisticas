...
class AlbumSale(models.Model):
	...
	def __str__(self):
		return f"{self.album_id} · {self.units}u · {self.amount}{self.currency}"
...
class Rating(models.Model):
	...
	class Meta:
		# Allow multiple individual ratings per user/song (each save is a new record).
		# NOTE: The previous unique constraint enforcing one rating per (user, song)
		# has been intentionally removed so the system accepts multiple ratings
		# per user/song. If you need to re-enable it later, add a
		# UniqueConstraint on ['user', 'song_id'] and create the corresponding
		# migration.
		indexes = [
